...
const EditCraft = () => {
  ...
  const onSubmit = async values => {
    setIsLoading(true)
    const toastId = toast.loading("Updating craft...")
    try {
      const res = await fetch(`http://localhost:4000/craft/${craft._id}`, {
        method: "PUT",
        headers: {
      ...
      if (res.ok) {
        navigate("/my-craft")
        toast.success("Craft update successfully!", { id: toastId })
      }
    } catch (error) {
      toast.error("Unable to update craft", { id: toastId })
    } finally {
      setIsLoading(false)
    }
  }
  ...
  if (user.email !== craft.user_email) {
  ...
  return (
    <div className="px-4 flex-1 flex flex-col items-center justify-center">
      <SectionHeader title="Update Craft Item" />
